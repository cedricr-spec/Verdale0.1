...
function createDroppedItemEntity(x, y, type = ENTITY_TYPES.RESOURCE, options = {}) {
  return {
    id: createEntityId("drop"),
    x,
    y,
    type,
    itemKey: options.itemKey,
    spriteKey: options.spriteKey,
    reward: options.reward || "inventory_item",
    rewardAmount: Math.max(1, Math.floor(options.rewardAmount ?? 1)),
    collectEffects: options.collectEffects,
    active: true,
    source: options.source || "throw",
    droppedAt: options.droppedAt ?? Date.now(),
    pickupDisabledUntil: options.pickupDisabledUntil ?? 0,
  };
}
...
export const createEntitySlice = (set) => ({
  ...
  spawnDroppedItem: (x, y, type = ENTITY_TYPES.RESOURCE, options = {}) => {
    const entity = createDroppedItemEntity(x, y, type, options);
    set((state) => ({
